Hi there, I'm Ghassen Faidi 👋

### About me
- Studying rigorous computer science & software engineering at university  
- I'm interested in education, metacognition, technical writing and translation
- I have a [personal blog](https://faidi.tech/)
- I watch some old Japanese films from time to time (Ozu/Kurosawa)
- I really like tea, but I haven't mastered making it yet.

### 🌱 Currently 
- I'm learning how to draw!
- Studying Tim Roughgarden's Algorithms course
- Learning Docker (fullstackopen)
- Learning UI/UX
- Learning French (again)

### OSSU Progress
- [ ] Introduction to Computer Science and Programming using Python
- [x] Systematic Program Design
- [ ] Class-based Program Design -- started did not complete yet
- [x] Programming Languages
- [ ] Object-Oriented Design
- [ ] Software Architecture
- [ ] The Missing Semester of Your CS Education -- started did not complete yet
- [x] N2T part 1
- [ ] N2T part 2
- [x] Operating Systems: Three Easy Pieces
- [x] Computer Networking: a Top-Down Approach
- [ ] Algorithms --  started did not complete yet
- [ ] Cyber Security MOOC
- [ ] Secure Coding Specialization
- [x] Databases
- [x] Machine Learning
- [ ] Computer Graphics
- [ ] Software Engineering
- [ ] Ethics, Technology and Engineering
- [ ] Introduction to Intellectual Property
- [ ] Data Privacy Fundamentals
- [ ] Software Debugging
- [ ] Software Testing
- [ ] Crafting Interpreters
- [ ] Advanced Information Security
- [x] Fullstack Open
- [ ] Functional Programming in Scala




